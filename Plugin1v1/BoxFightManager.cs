...
namespace Plugin1v1
{
    ...
    public class BoxFightManager : MonoBehaviour
    {
        ...
        public bool AcceptInvite(Player ply)
        {
            ...
            foreach (var kokot in Invites)
            {
                if (kokot == null)
                {
                    playerToRemove.Add(kokot);
                    continue;
                }
                if (kokot == ply)
                {
                    if (kokot.FightManager().IsFighting)
                    {
                    ...
                    foreach (var kokotnadruhu in playerToRemove)
                    {
                        Invites.Remove(kokotnadruhu);
                    }
                    return true;
                }
            }

            foreach (var kokot in playerToRemove)
            {
                Invites.Remove(kokot);
            }
        ...
        {
            List<FightMap> kokotkovia = new List<FightMap>();
            foreach (var kokot in possibleMaps)
            {
                if (!kokot.inUse)
                {
                    kokotkovia.Add(kokot);
                }
            }
            var selectedMapka = kokotkovia[UnityEngine.Random.Range(0, kokotkovia.Count)];
